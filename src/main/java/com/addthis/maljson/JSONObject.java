...
public class JSONObject {
    ...
    private final Map<String,LineNumberInfo> lineNumberKeys;// = new HashMap<String, LineNumberInfo>();
    private final Map<String,LineNumberInfo> lineNumberValues;// = new HashMap<String, LineNumberInfo>();
    private LineNumberInfo lineNumberInfo = LineNumberInfo.MissingInfo;
    ...
    public JSONObject() {
        ...
        lineNumberKeys = new HashMap<String, LineNumberInfo>();
        lineNumberValues = new HashMap<String, LineNumberInfo>();
    }
    ...
    public JSONObject(JSONTokener readFrom) throws JSONException {
         ...
         */
        LineNumberInfo lineInfo = readFrom.getLineNumberInfo();
        Object object = readFrom.nextValue();
        if (object instanceof JSONObject) {
            this.nameValuePairs = ((JSONObject) object).nameValuePairs;
            this.lineNumberKeys = ((JSONObject) object).lineNumberKeys;
            this.lineNumberValues = ((JSONObject) object).lineNumberValues;
        } else {
            throw JSON.typeMismatch(object, "JSONObject", lineInfo);
        }
    }
    ...
    public JSONObject put(String name, Object value) throws JSONException {
        return put(name, value, LineNumberInfo.MissingInfo, LineNumberInfo.MissingInfo);
    }

    public JSONObject put(String name, Object value, LineNumberInfo keyInfo, LineNumberInfo valInfo) throws JSONException {
        if (value == null) {
            nameValuePairs.remove(checkName(name));
            lineNumberKeys.remove(name);
            lineNumberValues.remove(name);
            return this;
        }
    ...
    // Missing method from public javadoc documnted
    // http://code.google.com/p/android/issues/detail?id=63338&
    ...
    /**
     * Get the key line number information associated with a key.
     *
     * @param key   A key string.
     * @return      The LineNumberInfo associated with the key in the (key,val).
     */
    public LineNumberInfo getKeyLineNumber(String key)  {
        LineNumberInfo info = lineNumberKeys.get(key);
        return info == null ? LineNumberInfo.MissingInfo : info;
    }

    /**
     * Get the value line number information associated with a key.
     *
     * @param key   A key string.
     * @return      The LineNumberInfo associated with the val in the (key,val).
     */
    public LineNumberInfo getValLineNumber(String key)  {
        LineNumberInfo info = lineNumberValues.get(key);
        return info == null ? LineNumberInfo.MissingInfo : info;
    }
